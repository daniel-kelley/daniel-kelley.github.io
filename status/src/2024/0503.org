#+TITLE: 27 April 2024 - 03 May 2024

* MIT OCW
** 18.06sc (Linear Algebra)
*** Unit II: Projection onto Subspaces
* SJ grounds
Some weed pulling.
* SCZ Front Step
** Identified piece of chactacote that should work for door sill
* Jointer
Rewiring. 60 year old 3-wire rubber coated cord disintegrated. Needed
for SCZ door sill project.
* gaiasky
** broken from arch updates (or something...)
** HDF stuff continuing
Memory problems addressed.
* Andromeda Path analysis
** "fun" reconciling EDD catalog with GAIA DR3
** Have query interface to "real" GAIA DR3 data.
* uri-cache
Needed update to accommodate long query strings from GAIA DR3 queries.
* piper
OS update broke mail setup. Need a better way to check system status
after updating.
* into the west
Needed a musical exercise, so transcribing song into lilypond. This
also gets an audio workflow st up on arch linux.
* tbattery
