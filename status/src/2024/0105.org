#+TITLE: 30 December 2023 - 05 January 2024
...
Textbook arrived.
* status/src
** Update status/src/tools/next to handle year change
* bwmse
...
Seems the easiest thing to do was a regex based line parser. Only took
four different parsers to reach that conclusion.
** Started hand written WSN parser
Will use for C++ standard grammar.
* kiki/arch
** Resolve funky tmux split screen
Ultimately lead to "reverting" back to xterm after 20 years of
rxvt. Maybe it hasn't been that long but it sure seems like it. The
world has moved on to UTF8, so I need to finally do that too. Farewell
LANG=C!
* Various i686
*** netbsd
installed.
