#+TITLE: 30 November 2024 - 06 December 2024
...
Declared done.
* Sonic Fiber
** need to decide to expose network or not
** bind notifies not working
need to debug
* GMPASM assembler
** parser unit tests started
** can parse asxxxx PDP11, 6502, 1802 test code with minimal changes
** can parse RISC-V code with no changes
That would not be strictly true if I implemented the grammar correctly.
However, an implementation mistake revealed how to update the parser
to handle it. Yay! for serendipitous mistakes.
* rebnf
