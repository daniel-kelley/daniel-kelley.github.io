#+TITLE: 20 April 2024 - 26 April 2024

* MIT OCW
** 18.06sc (Linear Algebra)
*** Unit II: Orthogonal Vectors and Subspaces
* SJ grounds
Mowed.
* SCZ Front Step
** Fitted door sill
Will use fitted sill as template for final.
* gaiasky
** broken from arch updates (or something...)
** HDF stuff continuing
Memory problems addressed.
** Path analysis
*** Current path using gaia dr3 subset is all stars.
*** Seeing if EDD catalog is suitable for galaxies.
*** Have query interface to "real" GAIA DR3 data.
* ca-r
Data no longer being updated, so weekly builds are stopped.
* xz compromise
Kaveri still need updating.
