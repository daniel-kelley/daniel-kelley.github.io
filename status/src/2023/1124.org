#+TITLE: 18 November 2023 - 24 November 2023

* MIT OCW
** 18.03sc (differential equations).
*** Unit IV - Matrix Exponentials
* wood chips
Continuing. Slowly
* SCZ Front Step
** Door sill to SJ to clean up
*** Will try to fix mistake instead of replace
* Next arch installs
** M22 (AMD kavari box)
Done. Actually named 'kavari'. Doesn't follow the 'dog name' theme,
but it's what I called it anyway.
** Various i686
*** Arch/32 requires too much memory (512Mb) for what I have (384Mb).
*** Will try Alpine linux or (gasp!) netbsd.
* opencl
...
*** Forces my hand to replace the server.
*** Updating configuration to the 21st century.
*** Mail being received.
...
** Use a regular keyboard for input
Have more than enough laying around.
** Use an LCD panel for output
Also have more than enough. Old enough hardware can drive the display
directly off the parallel port.
