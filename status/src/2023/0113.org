#+TITLE: 07 January 2023 - 13 January 2023

* MIT OCW
** Started 18.03sc (differential equations).
*** Unit I - SCF11 Part I Problems
* Mill Controller #2
...
** Server down on 03 January. Back up 11 Jan due to weather.
*** BIOS misconfigured. Fixed.
* Abby
** Added 4TB HDD
** Updated OS to Fedora 37
* Virtual Orchestra
** sfz file tests
...
caps on ISA board.
* Ada/SPARK
gnat-llvm requires BITS_PER_UNIT to be 8 as far as I can tell.  Byte
addressability seems to be deeply embedded in GCC
infrastructure. Continuing investigation in how to support
non-standard BITS_PER_UNIT for Ada. Although ACK doesn't help directly
because it assumes targets are byte addressable, it doesn't appear
that this restriction is architectural, just implementation.  Looking
into ACK-EM GCC backend if there's a way to convey Ada parameters to
it and use a custom EM translator for non-byte-addressable subtargets.
In any case, an ACK-EM target may be useful onit's own.
* POSIT
Use POSITs for YRX?
* MAME
