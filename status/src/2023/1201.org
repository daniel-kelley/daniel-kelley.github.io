#+TITLE: 25 November 2023 - 01 December 2023
...
Continuing. Slowly
* SCZ Front Step
** Door sill to SJ to clean up
*** Patch glued in
* Next arch installs
** Various i686
*** Arch/32 requires too much memory (512Mb) for what I have (384Mb).
*** Alpine linux
Don't like it.
*** netbsd
Not sure *I* have the bandwidth right now.
*** buildroot
Thinking about it.
** cindy
Woefully underpowered HP 14-dq0 notebook. Four mighty (de-)Celeron
cores and 64 whole gigabytes of SSD. Mom got it but hated it, so I
have it now.
** kiki
Wasted half a day trying to get the BIOS updated in the vain hope of
fixing access to the TPM. Total failure. I Did learn that maybe I
should check the fwupd website to see if potential hardware purchases
are supported.  The trick is going to be remembering to do this.
* opencl
...
** Looks like a power fluctuation took out 'panji'
*** apache running
*** roundcubemail installed
Access in internal network only. Currently via SSH tunnel.
*** Working on updating bind configurtion.
PITA if one is paranoid. Good thing it's a hidden master setup.
