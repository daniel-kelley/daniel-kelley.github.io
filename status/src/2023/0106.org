#+TITLE: 01 January 2023 - 06 January 2023
...
** Server down on 03 January. Need to check.
* Virtual Orchestra
** sfz file tests
...
caps on ISA board.
* Ada/SPARK
Investigating Ada (and SPARK subset) for ersatz CPU compiler
frontends.  Idea is to use gnat-llvm to issue LLVM asm IR and
transcode IR to target. Will depend on how flexible
System.Storage_Unit is going to be (I think). ACK doesn't help because
is assumes targets are byte addressable.
* POSIT
Use POSITs for YRX?
* MAME
