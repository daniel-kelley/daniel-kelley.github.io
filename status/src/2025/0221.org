#+TITLE: 15 February 2025 - 21 February 2025

* MIT OCW
** 18.05 - Introduction to Probability and Statistics
*** Suspended
Will refocus on combinatorics. Still have assembler-on-the-brain, so
will readdress when the assembler is not all consuming.
* Sonic Fiber
** rsyncing status page to g1
better then nothing or waiting a week to get t SJ to update the
address.
...
** Works with RIOT OS.
* GMPASM assembler
** lex: use own pushback buffer
** Use backtick for frame variable reference
** Compound prefix decorators
** Improve '%' and '(' handling used in factor and register tokens
** .type, .error, .warning
Now can do some tests in GMPASM assembly
** Detect unterminated macro constructs
...
than other stuff?
*** STM32f676zi
Could get ~32 PWMs of various quality on board. If it's possible, seems like
