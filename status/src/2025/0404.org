#+TITLE: 29 March 2025 - 04 April 2025
...
** separate label and op exec lists
labels were getting combined with macro expansion which was screwing up
local label state.
** octal constant lexer cleanup
** make stmt struct private
** .fill handling for V_ADDR
** more test cases
* Collatz conjecture
Some study and test programs.
* Programmable power supply
Started looking at this again; considering different appoaches.
* Raspberry Pi Pico
Studying Pico architecture. Looks like 16 usable PWMs on a $5
card. More straightforward than stm32f767zi. Parts ordered.
* Github website
Added commit hook to stop commits to main branch.
* Sonic Fiber
